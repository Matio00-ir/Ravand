      ...
      <div className="min-w-0 space-y-6">
        ...
        <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
          ...
          <Panel>
            ...
            <div className="mt-6 text-silver">
              <Donut
                segments={segments}
                centerValue={o("composition.center")}
                centerLabel={o("composition.centerLabel")}
              />
            </div>
          </Panel>
        </div>
        ...
        <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
            ...
            >
              {rows.map((row) => (
                <Tr key={row.ref}>
                  ...
                  <Td>
                    <Status tone={docTone[row.status] ?? "neutral"}>
                      {o(`documents.status.${row.status}`)}
                    </Status>
                  </Td>
                </Tr>
              ))}
          ...
          <Panel flush>
            <PanelHead
              className="p-5"
              title={o("approvals.title")}
              meta={o("approvals.meta")}
            />
            <ul className="border-t border-line-dark">
              {approvals.map((item) => (
                <li
                  key={item.title}
                  className="border-b border-line-dark px-5 py-4 last:border-b-0"
                >
